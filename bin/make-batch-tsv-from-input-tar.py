...
def main():
    ...
    # Write header
    out_fh.write("--input INPUT\t--output-recursive OUTPUT_PATH\t--env SAMPLE_ID\t--env SCHEMA\t--env SERIES\n")
    with open(input_file, 'r') as in_fh:
        for line in in_fh:
            in_path = line.strip()
            path_elements = in_path.split('/')
            sample_id = path_elements[4]
            sample_id = sample_id.split('.')[0]	# by Jina (05/31/2018) with chr number
            out_path = gs_out_dir	# by Jina (07/17/2018) just assign the output path for unpacking the tar files
            #out_path = "{0}/{1}/{2}".format(gs_out_dir, sample_id, gs_out_suffix)
